...
const BUILTIN = [
  ...
  { icon: '📝', title: '待办事项', desc: '打开 LinkHub 待办页面', type: '页面', actionType: 'page', page: 'todo' },
...
// 获取常用网站命令（数据来自 newtab/js/data.js）
function getSiteCommands() {
  const categories = window.LinkHubData?.devSiteCategories || [];
  const commands = [];
...
async function buildItems() {
  ...
  const sites = getSiteCommands();
  _items = [...BUILTIN, ...sites, ...servers, ...bookmarks];
}
